...
for i in tests/minutos_test/*.txt; do
	echo "Compiling: $i"
...
for i in compiled_test/*.fst; do
	...
	fstcompose compiled_test/$(basename $i) compiled/minutos.fst | fstshortestpath | fstproject --project_type=output | fstrmepsilon | fsttopsort | fstprint --acceptor --isymbols=./syms.txt
done
